...
const AdminChatDetail = () => {
  ...
  const targetUserId = Array.isArray(id) ? id[0] : id; // Normalize to string
  ...
  const setUserNameSafe = (name: string) => {
    ...
    async (showLoading = true) => {
      ...
      try {
        const res = await fetch(
          `${SERVER_URL}/messages/history/${targetUserId}`,
        );
        ...
        if (Array.isArray(data)) {
          ...
          const firstUserMsg = data.find(
            (m: any) => m.role === "USER" && m.fromName,
          );
          if (firstUserMsg) {
            setUserNameSafe(firstUserMsg.fromName);
          }
  ...
  useEffect(() => {
    ...
    const handleMessage = (msg: Message | any) => {
      ...
      // - user sent it to admin (msg.from === targetUserId)
      // - admin sent it to this user (msg.to === targetUserId)
      const belongsHere = msg.from === targetUserId || msg.to === targetUserId;
      if (!belongsHere) return;

      if (msg.fromName) setUserNameSafe(msg.fromName);

      setMessages((prev) => {
        if (prev.some((m) => m.id === msg.id)) return prev;
        // Replace optimistic message (matched by content + role) with real server message
  ...
  const sendMessage = () => {
    ...
    // Optimistic update — use temp- prefix so handleMessage can replace it on echo
    ...
    socketRef.current.emit("send_message", { content, to: targetUserId });
    socketRef.current.emit("stop_typing", { to: targetUserId });
    setInputText("");
  ...
  const renderItem = ({ item }: { item: Message }) => {
    ...
    return (
      <View
        className={`max-w-[80%] p-4 rounded-[20px] mb-3 flex-col ${
          isMe
    ...
    >
          ...
          ListEmptyComponent={
            <View className="items-center justify-center pt-20">
              ...
              <Text className="text-gray-400 font-bold mt-4">
                {t("admin.messages.startConversation")}
              </Text>
            </View>
          }
        />
      ...
      {isUserTyping && (
        <View className="px-5 py-2">
          <TypingIndicator
            label={`${t("admin.messages.user")} ${t("common.isTyping")}`}
          />
        </View>
      )}
